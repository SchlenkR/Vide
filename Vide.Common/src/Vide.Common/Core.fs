...
type Vide<'v,'s,'c> = Vide of ('s option -> IHost -> 'c -> 'v * 's option)
    ...
    let bind<'v1,'s1,'v2,'s2,'c>
        ...
        Vide <| fun s host ctx ->
            let ms,fs =
                ...
                | None -> None,None
                | Some (ms,fs) -> ms,fs
            let mv,ms = m ms host ctx
            let (Vide f) = f mv
            let fv,fs = f fs host ctx
            fv, Some (ms,fs)
    ...
    let combine<'v1,'s1,'v2,'s2,'c>
        ...
        Vide <| fun s host ctx ->
            let sa,sb =
                ...
                | None -> None,None
                | Some (ms,fs) -> ms,fs
            let va,sa = a sa host ctx
            let vb,sb = b sb host ctx
            vb, Some (sa,sb)
