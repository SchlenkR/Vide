...
let cacheDir = Path.Combine(__SOURCE_DIRECTORY__, ".cache/mdnCache")
...
module Helper =
    ...
    module Serialize =
        ...
        let serialize (o: obj) =
            JsonSerializer.Serialize(o, makeOptions ())

        let serializeNoIndent (o: obj) =
...
module MdTest =
    ...
    let domIfMap =
        result.elements 
        |> List.map (fun e -> e.name, e.domInterfaceName)
        |> List.map (fun (name,dif) -> $"""  "{name}", "{dif}" """)
        |> String.concat "\n"
        |> fun s -> $"[ \n {s} \n ]"
